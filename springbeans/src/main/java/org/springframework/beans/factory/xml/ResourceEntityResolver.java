...
/**
 * {@link org.xml.sax.EntityResolver}的实现，这个是xml相关的。主要用来通过
 * {@link ResourceLoader}解析entity引用,通常，都是基于applicationContext的resource基础目录。
 * 继承了 {@link DelegatingEntityResolver}提供DTD和XSD查找功能。
 *
 ...
 *
 * 允许通过标准的xml entity去包含xml 片段到应用上下文，比如，分拆一个大的xml文件到
 * 不同的模块。这个包含路径可以是基于应用上下文的resource基础路径，而不用是基于JVM工作路径的
 * 相对路径。
 * 关于publicId和systemId，可以看看这个链接，
 * https://blog.csdn.net/agony_sun/article/details/72903722
 *
