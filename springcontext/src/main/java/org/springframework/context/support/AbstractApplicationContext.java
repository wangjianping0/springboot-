...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				/**
				 * {@link org.springframework.bootstrap.context.embedded.EmbeddedWebApplicationContext}
				 */
				...
				/**
				 * 这里初始化事件发布器，里面聚合了beanFactory、listener列表，
				 * 该事件发布器可以发布事件，listener可以做对应处理
				 */
				...
				List<BeanDefinition> beanDefinitionList = beanFactory.getBeanDefinitionList();
                ArrayList<BeanDefinition> arrayList = new ArrayList<>();
                for (BeanDefinition beanDefinition : beanDefinitionList) {
                    boolean b = beanDefinition.getDependsOn() != null
                            && beanDefinition.getDependsOn().length > 0;
                    if (b) {
	 ...
	 * 配置factory的特点，比如classloader、后置处理器
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 添加一个后置处理器，用来给实现了Aware接口的bean，注入对应的bean，比如实现了applicationContextAware的bean，
		 * 要注入applicationContext；
		 * 其他的要处理的Aware接口包括：EnvironmentAware EmbeddedValueResolverAware ResourceLoaderAware ApplicationEventPublisherAware
		 * MessageSourceAware
		 */
		...
		/**
		 * 因为this，在spring boot的web项目里，一般就是
		 * org.springframework.bootstrap.context.embedded.AnnotationConfigEmbeddedWebApplicationContext
		 * 这个类，实现了ResourceLoader ApplicationEventPublisher ApplicationContext 这些接口，所以，
		 * 如果遇到有些bean依赖这几个类的接口，则直接将this注入即可
		 */
		...
		/**
		 * 如果当前beanFactory包含了 运行时织入bean，则加入一个bean后置处理器：
		 * LoadTimeWeaverAwareProcessor，如果bean实现了 LoadTimeWeaverAware，则给bean，注入 LoadTimeWeaver
		 */
		...
		/**
		 * 判断当前bean工厂（不往祖先去查找）是否包含了 environment 这个bean，如果不包含，则注册一个
		 */
		if (!beanFactory.containsLocalBean(ENVIRONMENT_BEAN_NAME)) {
			beanFactory.registerSingleton(ENVIRONMENT_BEAN_NAME, getEnvironment());
		}
		/**
		 * 注册systemProperties bean
		 */
